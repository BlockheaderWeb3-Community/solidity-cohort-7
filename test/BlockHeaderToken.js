  ...
  describe("Transactions", () => {
    describe("Transfers", () => {
      it("Should transfer tokens from one account to another", async () => {
        ...
        await BlockToken.connect(_owner).transfer(addr1, 700);
        expect(await BlockToken.balanceOf(_owner)).to.eq(300);
      ...
      it("Should revert if address 'to' is address zero", async () => {
        const { BlockToken, _owner, addr1, addr2 } = await loadFixture(
          deployContract
        );
        const zeroAddress = "0x0000000000000000000000000000000000000000";
      ...
      it("Should revert if allowance is increased and not replaced", async () => {
        const { BlockToken, _owner, addr1, addr2 } = await loadFixture(
          deployContract
        );
