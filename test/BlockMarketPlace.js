  ...
  describe("Deployment", () => {
    ...
    describe("BuyNFT", ()=>{
      it("Should revert if the NFT is already sold", async ()=>{
        ...
        it("Should buy successfully with ERC20 token", async()=>{
          const{marketplace, owner_, blocknft, BlockToken, addr1, addr2} =
            await loadFixture(deployBlockMarketPlace);
          let tokenId = 1;
          await blocknft.connect(addr1).mint(addr1);
          let token = await ethers.getContractAt("IERC20", BlockToken);
        ...
        it("Should buy successfully with native ETH", async()=>{
              ...
              expect(await blocknft.ownerOf(tokenId)).to.eq(addr2.address);
              expect((await marketplace.getListing(listId)).sold).to.eq(true);
        })
        ...
        it("Should revert if the price is incorrect", async()=>{
                ...
                await expect(marketplace.connect(addr2).buyNft(listId, {value: 7})).
    ...
    describe("Offers", ()=>{
      it("Should revert if offer is placed for an already sold Listing", async()=>{
            const { marketplace, addr1, blocknft, addr2, addr3 } = await loadFixture(
              ...
              await expect(marketplace.connect(addr3).offer(listId, 700)).to.be.revertedWith("Already sold");
            
      })

      it("Should revert if msg.value is less tham min offer", async()=>{
            const { marketplace, addr1, blocknft, addr2 } = await loadFixture(
                deployBlockMarketPlace
              );
              ...
              await expect(marketplace.connect(addr2).offer(listId,0, {value: 3})).to.be.revertedWith("Invalid offer")
      })

      it("Should revert if offerAmount is not 0 when offering", async()=>{
            const { marketplace, addr1, blocknft, addr2 } = await loadFixture(
                deployBlockMarketPlace
              );
              ...
              await expect(marketplace.connect(addr2).offer(listId,1, {value: 6})).to.be.revertedWith("Cannot offer erc20")
      })

      it("Should revert if offerAmount == msg.value when offering with native ETH", async()=>{
            const { marketplace, addr1, blocknft, addr2 } = await loadFixture(
                deployBlockMarketPlace
              );
              ...
              expect((await marketplace.connect(addr2).getOffer(0)).offerAmount).to.eq(7);
            ...
            it("Should check that msg.sender != listing owner", async()=>{
              ...
              await expect(marketplace.connect(addr1).offer(listId, 0, {value: 7}))
    ...
    describe("AcceptOffers", ()=>{
      it("Should revert if owner is not msg.sender", async()=>{
        const { marketplace, addr1, blocknft, addr2 } = await loadFixture(
              ...
              await marketplace.connect(addr2).offer(listId,0,{value: 10});
              expect((await marketplace.getOffer(0)).offerAmount).to.be.eq(10);

              await expect(marketplace.connect(addr2).acceptOffer(0)).to.be.revertedWith("Unauthorized seller");
    ...
    describe("CancelOffers", ()=> {
      it("Should revert if trying to cancel already accepted offer", async()=>{
            ...
            await expect(marketplace.connect(addr2).cancelOffer(0)).to.be.revertedWith("Offer already accepted");
      
      })

      it("Should revert if not offerrer trying to cancel offer", async()=>{
        const { marketplace, addr1, BlockToken, blocknft, addr2, owner_ } =
              await loadFixture(deployBlockMarketPlace);
            ...
            await expect(marketplace.connect(addr1).cancelOffer(0)).to.be.revertedWith("Unauthorized offerrer");
            
            await marketplace.connect(addr2).cancelOffer(0);
      
      })

      it("Should cancel offer succesfully", async ()=>{
        const { marketplace, addr1, BlockToken, blocknft, addr2, owner_ } =
              await loadFixture(deployBlockMarketPlace);
    ...
    describe("CancelListings", () =>{
        it("Should revert if unathorized user tries to cancel listing", async()=>{
          const { marketplace, addr1, blocknft, addr2} =
              await loadFixture(deployBlockMarketPlace);
        ...
        it("Should revert if unathorized lister tries to cancel listing", async()=>{
         const { marketplace, addr1, blocknft, addr2 } = await loadFixture(
                 deployBlockMarketPlace
               );
