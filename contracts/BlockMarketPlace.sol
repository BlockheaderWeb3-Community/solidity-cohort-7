...
contract BlockMarketPlace {
    ...
    function listNft(Listing memory list) external {
        ...
        if (list.isNative) {
            require(
                address(list.paymentToken) == address(0),
                "ERC20 Payment is not supported"
            );
        }
        ...
        IERC721(list.NftToken).transferFrom(
            msg.sender,
            address(this),
            list.tokenId
        );
    ...
    //getter function for offerDetailStruct
    function getOffer(
        uint256 offerId
    ) external view returns (OfferDetails memory o) {
        o = idToOffer[offerId];
    }
    ...
    function acceptOffer(uint256 offerid) external {
        ...
        // Interactions
        if (l.isNative) {
            (bool success, ) = l.owner.call{
                value: (offer_.offerAmount * 97) / 100
            }("");
            (bool success2, ) = marketOwner.call{
                value: (offer_.offerAmount * 3) / 100
            }("");
            ...
            l.paymentToken.transfer(
                marketOwner,
                (offer_.offerAmount * 3) / 100
            );
        }
        IERC721(l.NftToken).transferFrom(
            address(this),
            offer_.offerrer,
            l.tokenId
        );
