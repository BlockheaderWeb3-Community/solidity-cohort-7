...
contract BlockToken is ERC20 {
    ...
    function mint(uint256 _amount, address _recepient) external onlyOwner notAmount0(_amount) {
        _mint(_recepient, _amount);
    }

    function burn(uint256 _amount) external notAmount0(_amount) {
        _burn(msg.sender, _amount);
    }

    function burnFrom(address _user, uint256 _amount) external onlyOwner notAmount0(_amount) {
        _burn(_user, _amount);
    }
}
